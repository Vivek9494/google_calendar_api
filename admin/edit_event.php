...
if(!empty($_GET)){
    ...
    if($rows1 == 0){
        ...
        $event = mysqli_fetch_array($query1);
    }
}
...
if(isset($_POST['edit_event_btn'])) {
    ...
    if(empty($error_messages)){
        ...
        $sql = "SELECT * FROM events WHERE event_name = '$event_name' AND start_date = '$start_date' AND end_date = '$end_date' AND status = 1 AND id != '$id'";
        ...
        if(!empty($email_exist)){
            ...
            $sql = "UPDATE events SET event_name= '$event_name',start_date = '$start_date',end_date = '$end_date' WHERE id='$id'";
            $query = mysqli_query($conn,$sql);
    
            $_SESSION['user']['event_action'] = 'update';
            $_SESSION['user']['last_inserted_event_id'] = $id;
                ...
                <section class="content">
                   <!-- general form elements -->
                    <div class="card card-primary">
                        <!-- form start -->
                        <form role="form" name="edit_event_frm" id="edit_event_frm" action="edit_event.php" method="post">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <label>Event</label>
                                    <input type="text" class="form-control" id="event" name="event" placeholder="Enter Event Name" value="<?php echo (!empty($_POST['event'])) ? $_POST['event'] : $event['event_name'];?>">
                                </div>
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Start Date</label>
                                            <input type="text" class="form-control" id="start_date" name="start_date" placeholder="Start Date" value="<?php echo (!empty($_POST['start_date'])) ? $_POST['start_date'] : date('m/d/Y',strtotime($event['start_date']));?>">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="end_date" name="end_date" placeholder="End Date" value="<?php echo (!empty($_POST['end_date']))  ? $_POST['end_date'] : date('m/d/Y',strtotime($event['end_date']));?>">
                            ...
                            <div class="card-footer">
                                <button type="submit" class="btn btn-primary" name="edit_event_btn">Submit</button>
                            </div>
                        </form>
                    </div>
                    <!-- /.card -->
